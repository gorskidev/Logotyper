          ...
          this.state.suggestion.map((element, index) => 
            { 
              if(index < 6) {
                ...
                <Link to={this.parseToGalleryUrl(element.title, element.author, element.price, element.imgPath, element.id)} key={element._id}>
                  <div className="suggestion" key={element._id} onClick={this.handleClick}>
                    <span><img src={element.imgPath}/></span> {element.title} by {element.author}
                  </div>
                </Link>
                )
              }
